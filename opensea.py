...
class SetForSaleOnOpensea(unittest.TestCase):
    def setUp(self):
        ...
        if self.SECRET_WORDS == "" or self.SECRET_WORDS == None:
            driver.quit()
            raise Exception(
                "Secret phrase not defined. Please provide wallet secret phrase as one string in .env file"
            )
        if self.PASSWORD == "" or self.PASSWORD == None:
            driver.quit()
            raise Exception(
                "Password not defined. Please provide password in the .env file"
            )
    ...
    def typeSecretWords(self, secretWords):
        ...
        for i in range(len(listOfSecretWords)):
            driver.find_element(
                By.XPATH, f'//*[@id="import-srp__srp-word-{i}"]'
            ).send_keys(listOfSecretWords[i])
    ...
    def setForSale(self, chld, parent):
        ...
        for i in range(numOfNFTs):
            ...
            NFT.click()
            time.sleep(5)
            sellBtn = driver.find_element(By.XPATH, "//*[contains(text(), 'Sell')]")

            if sellBtn.is_displayed():
                sellBtn.click()
                WebDriverWait(driver, 10).until(
                ...
                driver.find_element(
                    By.XPATH, "//*[contains(text(), 'Complete listing')]"
                ).click()
                time.sleep(3)
                ...
                WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CSS_SELECTOR, 'i[title="Scroll down"]')
                    )
                )
                ...
                WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, 'button[data-testid="signature-sign-button"]')
                    )
                )
                driver.find_element(
                    By.CSS_SELECTOR, 'button[data-testid="signature-sign-button"]'
                ...
                WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CSS_SELECTOR, 'i[aria-label="Close"]')
                    )
                )
                driver.find_element(By.CSS_SELECTOR, 'i[aria-label="Close"]').click()
            else:
                print(f"Sell button not present for NFT: {NFT_ID}")
            ...
            # Go to profile page
            WebDriverWait(driver, 100).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, 'img[alt="Account"]'))
            )
            try:
                driver.find_element(By.CSS_SELECTOR, 'img[alt="Account"]').click()
            except:
    ...
    def test_SetForSaleOnOpensea(self):
        ...
        #  Wallet button
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, 'i[title="Wallet"]'))
        )
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, "//*[contains(text(), 'MetaMask')]")
            )
        )
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, '[data-testid="first-time-flow__button"]')
            )
        )
        driver.find_element(
            By.CSS_SELECTOR, '[data-testid="first-time-flow__button"]'
        ...
        driver.find_element(
            By.CSS_SELECTOR, '[data-testid="page-container-footer-cancel"]'
        ).click()
        driver.find_element(
            By.CSS_SELECTOR, '[data-testid="import-wallet-button"]'
        ).click()
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, '[data-testid="EOF-complete-button"]')
            )
        )
        driver.find_element(
            By.CSS_SELECTOR, '[data-testid="EOF-complete-button"]'
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, "//*[contains(text(), 'Show/hide')]")
            )
        )
        ...
        driver.find_element(
            By.XPATH,
            '//*[@id="app-content"]/div/div[3]/div/div[2]/div[2]/div[2]/div[8]/div[2]/div/label/div[1]',
        ).click()
        driver.find_element(By.CSS_SELECTOR, '[data-testid="network-display"]').click()
        time.sleep(0.5)
        driver.find_element(
            By.CSS_SELECTOR, '[data-testid="goerli-network-item"]'
        ).click()
        #  Close Metamask settings
        driver.find_element(
            By.CSS_SELECTOR,
            '[class="settings-page__header__title-container__close-button"]',
        ).click()
